function [U_x,U_y,gF_x,gF_y,dissipation] = right_hand_side_parallel(W,Vx,Vy,Vx_new,Vy_new,Vrx,Vry,dv,epsilon,C_gamma,gamma,Np)
...
parfor i = 1:Np
    ...
    A11 = C_gamma*(len+eps).^gamma.*(len.^2-(Vmid_x(i)-Vmid_x).^2);
    A12 = -C_gamma*(len+eps).^gamma.*(Vmid_x(i)-Vmid_x).*(Vmid_y(i)-Vmid_y);
    
    A21 = A12;
    A22 = C_gamma*(len+eps).^gamma.*(len.^2-(Vmid_y(i)-Vmid_y).^2);
