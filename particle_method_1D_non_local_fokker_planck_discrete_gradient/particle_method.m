...
%     drawnow
    ...
    for i = 1:n
        ...
        w_term(i) = sum(.5*w.*(x(i)-x).^2);
    end
    
    eta = dx*(sum(inside.*log(inside)) + .5*sum(w.*w_term));
    error_list(nt,5) = rho;
    error_list(nt,6) = eta;
...
filename = ['data/n_',num2str(n),'_dx',num2str(dx),...
    '_dt',num2str(dt),'_t0_',num2str(t0),'_tmax_',num2str(time),'.mat'];
save(filename)
